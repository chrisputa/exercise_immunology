...
**Learning objectives:**
- Distinguish FOR / NFOR / OTS by recovery time and position them on the hormesis dose-response curve.
- Recognise the paradoxically low submaximal + maximal lactate signature of OTS and explain why a graded test alone misleads.
- Apply the 110% IAS time-to-exhaustion test and state the threshold for OT suspicion.
- Distinguish sympathicotonic vs parasympathicotonic OT (Israel) and explain the unifying β-adrenergic receptor down-regulation mechanism.
...
**Learning objectives:**
- Explain the Gabriel 1998 CD45RO receptor-density finding and its specificity/sensitivity, and why EBV/CMV must be screened first.
- Summarise Smith's cytokine hypothesis (micro-trauma → monocyte → IL-1β/IL-6/TNF-α → sickness behaviour + HPA exhaustion) and map it to Selye GAS stage 3.
- Describe the OTS immune phenotype (↓ NK cytotoxicity, ↓ neutrophil burst, Th2 shift, ↓ sIgA, ↑ resting IL-6, URTI/EBV reactivation).
- Integrated from lecture-4: distinguish the normal myokine adaptation profile from overtraining red flags (sustained IL-6 at 48–72 h, absent IL-1ra/IL-10, paradoxical TNF-α) as failed resolution.
...
**Learning objectives:**
- Apply Gleeson's six criteria for an ideal OTS marker and explain why a blunted cortisol response outperforms resting cortisol.
- Use POMS / TDS / RESTQ-Sport for early psychological monitoring and interpret the inverted iceberg profile.
- Apply the ECSS/ACSM 2013 two-bout maximal exercise test to differentiate FOR / NFOR / OTS by the bout-2 hormonal pattern.
- Calculate Foster session-RPE load, monotony and strain, and recommend evidence-based prevention (rest, sleep, 8.5 g/kg/day CHO).
...
**Learning objectives:**
- Differentiate OTS from ME/CFS / Long COVID via the lactate-clearance signature (production vs clearance/oxidation defect).
- Explain the Brooks-shuttle lens: OTS breaks upstream sympathetic drive (machinery intact) vs post-infectious syndromes break downstream oxidative machinery.
- Recognise when graded exercise is contraindicated (PEM populations) and why pacing within the energy envelope is required instead.
- Identify mixed phenotypes (overtrained + infected) and the expanded work-up they require.
