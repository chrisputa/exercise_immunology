...
description: Develops a framework for the *standardised evaluation of symptoms and signs* of both exercise-induced and infection-based immunological stress regulation — providing validated assessment instruments, diagnostic algorithms, and practical decision tools for clinical and field application.
draft: false
---
...
1. [Two Pathways of Immunological Stress](#1-two-pathways-of-immunological-stress)
2. [The Assessment Toolkit — Symptoms, Signs, Laboratory](#2-the-assessment-toolkit--symptoms-signs-laboratory)
3. [Decision Algorithms and Integration](#3-decision-algorithms-and-integration)

---

## 1 Two Pathways of Immunological Stress

**Learning objectives:**
- State the clinical need for a standardised framework: the two stress sources co-occur, interact, and are frequently indistinguishable without systematic evaluation, and the framework must differentiate them, track their temporal dynamics, flag early warnings, and guide training/referral/return decisions (Puta et al. 2026; Schwellnus et al. 2022).
- Contrast exercise-induced and infection-based stress across all seven dimensions of the framework table — primary trigger, onset, temporal profile, hallmark immune change, key hormones, performance impact, resolution — and reproduce the table from memory.
- Distinguish a *symptom* (subjective, reported) from a *sign* (objective, observed), and explain why both are needed.
- Apply the "neck check" heuristic: above-neck symptoms without systemic features → reduced-intensity exercise may continue; below-neck or systemic features (fever ≥ 38.5 °C, myalgia, malaise, dyspnoea, lymphadenopathy, GI symptoms) → cessation and medical evaluation (Puta et al. 2026; Nieman and Wentz 2019).
- Read the eight-row differentiation table (muscle soreness, fatigue, resting HR, fever, CRP, lymphocyte count, NLR, performance loss) and assign a given presentation to the more likely pathway.
- Explain why the *temporal pattern* is often the decisive discriminator — exercise stress resolves within ≈ 24 h, infection persists and worsens with exertion — and connect this to post-exertional malaise as a red flag.

### 1.1 Rationale and Clinical Need

Immune system stress in physically active individuals arises from two partially overlapping but mechanistically distinct sources: **exercise-induced immunological stress** (acute biphasic responses, overtraining, intensity-dependent suppression, trace element redistribution) and **infection-based immunological stress** (innate and adaptive immune activation, systemic inflammation, post-infectious sequelae). In clinical and field practice, these two sources frequently co-occur, interact, and may be clinically indistinguishable without systematic evaluation.
...
### 1.2 Conceptual Framework: Two Pathways of Immunological Stress
...
Exercise-induced versus infection-based stress across key dimensions.
</Fragment>
</Table>

### 1.3 Symptom Differentiation: Exercise-Induced vs. Infection-Based
...
<Fragment slot="caption">
Differentiating exercise-induced from infection-based symptoms.
</Fragment>
</Table>
...
- Why is standardisation essential when assessing immunological stress in athletes?
- Define the difference between a "symptom" and a "sign" in clinical reasoning.
- Why is the temporal pattern of symptoms (e.g., delayed worsening) clinically meaningful?
- How does post-exertional malaise (PEM) differ from normal post-exercise fatigue?
- Which signs suggest exercise-induced immunological stress rather than ordinary fatigue?
...
## 2 The Assessment Toolkit — Symptoms, Signs, Laboratory

**Learning objectives:**
- Describe the WURSS-21/WURSS-11 for quantitative URTI-symptom monitoring (0–7 severity across sneezing, congestion, sore throat, cough, headache, myalgia, plus functional impact) and its role in supporting the neck check (Barrett et al. 2002, 2005).
- Describe the DALDA questionnaire as a composite stress-and-recovery index spanning training load, sleep, nutrition, soreness and mood.
- Apply the training-load and recovery instruments — TQR, session-RPE (RPE × duration), and POMS/brief POMS — and explain how the overtraining psychological cluster overlaps the prodrome of acute infection.
- Perform the five-domain physical examination: vital parameters (resting tachycardia ≥ 7 bpm above baseline as a sensitive early marker), lymphatic palpation (tenderness/enlargement → infection; splenomegaly must be excluded before contact-sport clearance), upper-airway inspection (tonsillar exudate + lymphadenopathy + fever → streptococcal/EBV), cardiopulmonary auscultation (friction rubs, arrhythmia, murmurs → myocarditis exclusion), and neuromuscular proxies (grip, balance, reaction time) (Halle et al. 2021; Puta et al. 2026).
- Apply the tiered laboratory ladder: **Level 1** screening (differential count with NLR/SII/SIRI, CRP/ESR, resting HRV, urinalysis); **Level 2** extended (trace-element panel adjusted for haemoconcentration, pathogen serology/PCR, CK/AST/ALT/creatinine/LDH, TSH); **Level 3** specialised immunology (NK count and cytotoxicity, T-cell subsets, complement C3/C4, salivary IgA, oxidative burst and proliferation assays) (Simon et al. 2026).
- Justify the escalation logic: why Level 2 is triggered by Level 1 abnormalities and Level 3 reserved for persistent or severe/recurrent presentations.

### 2.1 Upper Respiratory Tract Infection Screening
...
### 2.2 Training Load and Recovery Monitoring
...
### 2.3 Clinical Signs Relevant to Immunological Stress Evaluation
...
### 2.4 Laboratory Diagnostics: A Tiered Approach

The following tiered approach is proposed for the standardised evaluation of immunological stress, escalating from field-applicable screening to specialised immunology:
...
- Name one validated questionnaire for monitoring training stress and explain its core dimensions.
- How does a symptom score complement biomarker data in decision-making?
- Name two practical field tools for assessing autonomic recovery.
- How can wearable HR/HRV data complement standardised symptom scales?
- What are the limitations of self-report instruments, and how can they be mitigated?
...
## 3 Decision Algorithms and Integration

**Learning objectives:**
- Define a diagnostic algorithm and justify its value for reproducible, defensible return-to-play decisions.
- Execute **Algorithm A** (acute symptom onset): fever ≥ 37.5 °C or below-neck symptoms → stop and evaluate; above-neck only → reduce to ≤ 60 % V̇O₂max and monitor 24–48 h → symptom-free at 48 h → graded return, else Level 1 diagnostics.
- Execute **Algorithm B** (return-to-sport after confirmed infection): ≥ 10 days post-diagnosis with ≥ 3 symptom-free days; clinical examination; Level 1 labs (+ troponin if myocardial involvement suspected); the 5-step graded return (rest → light aerobic → moderate aerobic → sport-specific → full training → competition), each step ≥ 24 h.
- Apply the three trace-element interpretation principles in the post-exercise setting: no sampling within 2 h for selenium, copper and free zinc (and account for acute haemolysis in iron); haemoconcentration correction against resting reference ranges; and read NLR/SII/SIRI in temporal context so their biphasic post-exercise pattern is not misread as infection or overtraining (Simon et al. 2026).
- Operate the standardised evaluation checklist end to end — TQR < 13, WURSS-11 ≥ 7/session, session-RPE acute:chronic > 1.5, resting HR ≥ +7 bpm, POMS iceberg inversion, temperature ≥ 37.5 / ≥ 38.5 °C, CRP > 5 mg·L⁻¹ with symptoms, the trace-element timing rule, any new cardiopulmonary finding — and state the action each threshold triggers.
- Argue why a multi-domain assessment outperforms any single marker, and identify a scenario where a standardised algorithm should override clinical intuition and one where it should not.

### 3.1 Algorithm A: Acute Symptom Onset in Athletes
...
### 3.2 Algorithm B: Return-to-Sport Clearance After Confirmed Infection

Return-to-sport clearance integrates four elements:
...
### 3.3 Integration: Immunological Biomarkers in the Context of Trace Elements

The standardised laboratory evaluation in the post-exercise setting must account for the acute redistribution of trace elements. Key principles for interpretation:
...
### 3.4 Practical Summary: Standardised Evaluation Checklist

The following checklist integrates the preceding sections into a field-applicable tool:
...
<Fragment slot="caption">
Monitoring toolbox with thresholds and actions for return-to-training.
</Fragment>
</Table>
...
- What is a diagnostic algorithm, and why is it useful for return-to-play decisions?
- Describe one decision tool you would use to clear an athlete after a viral infection.
- Explain why an integrative (multi-domain) assessment outperforms single-marker approaches.
- Describe a clinical scenario where a standardised algorithm would override clinical intuition — and one where it would not.
- How would you document immunological stress in a structured medical record?
- How would you train coaches to use these standardised tools correctly?
- Name one ethical concern when monitoring athletes with continuous symptom tracking.
