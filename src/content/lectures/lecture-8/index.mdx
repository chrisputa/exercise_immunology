...
**Learning objectives:**
- State the paradigm shift: lactate is not a waste product of anaerobic glycolysis but is formed continuously under fully aerobic conditions, and name its three established roles — oxidative fuel (heart, brain, liver, red muscle), gluconeogenic precursor (Cori cycle), and signalling molecule (myokine/exerkine, "lactormone") (Brooks 2018).
- Quantify continuous aerobic production: ≈ 50 % of lactate is oxidatively used at rest, rising to ≈ 75–80 % during intense exercise (Brooks 2018).
- Distinguish the three nested shuttle systems — cell–cell (CCLS, glycolytic → oxidative cells along gradients), intracellular (ILS, cytosol → mitochondrial matrix within a cell), and organ–organ (systemic, muscle → liver/brain via blood) (Brooks 2018).
- Describe the transport machinery: monocarboxylate transporters MCT1 and MCT4 with the co-factor CD147, and the mitochondrial lactate-oxidation complex (LDH + MCT1 + CD147) that enables direct intracellular oxidation (Brooks 2018).
- Explain how isotope-tracer studies (¹³C-lactate) and arterial–venous difference measurements document continuous shuttling, and how the mitochondrial reticulum generates the gradients that drive intracellular flux; name histone lactylation and CREB signalling as novel epigenetic routes (Brooks et al. 2022).
- Restate the clinical translation: lactataemia as a "strain" (load) biomarker rather than a pathology marker; therapeutic lactate infusions (traumatic brain injury, heart failure, inflammation); training-induced MCT upregulation and clearance capacity (Brooks 2018).
...
**Learning objectives:**
- Explain the Hui et al. 2017 reinterpretation of aerobic metabolism: systemic ¹³C tracing shows lactate turnover flux exceeds glucose (×1.1 fed, ×2.5 fasted), lactate labels TCA intermediates extensively in all tissues, and glucose feeds the TCA cycle *indirectly* via circulating lactate in every tissue except the brain.
- State the central conclusion — glycolysis and the TCA cycle are decoupled at the level of lactate, allowing independent regulation of ATP production and glucose catabolism — and note the tumour finding (lactate exceeds glucose as TCA substrate in lung and pancreatic tumours).
- Describe the Cai et al. 2023 breakthrough: lactate enters the mitochondrial matrix and directly stimulates ETC activity *independently of its own metabolism*, shown because both L-lactate (LDH-metabolisable) and D-lactate (not) activate the ETC, and D-lactate reversibly, ATP-dependently suppresses the Warburg effect.
- Lay out the mechanism: lactate → matrix entry → complex-independent ETC activation → ↑ OXPHOS ATP → ATP-mediated suppression of glycolysis → ↑ pyruvate/alternative-substrate use, and explain why this lets trained athletes oxidise efficiently at high lactate concentrations.
- State the immunological result — in primary T cells, D-lactate enhanced proliferation and effector function — and connect it to immune-cell activation after exercise.
...
**Learning objectives:**
- Describe the Leija et al. 2024 design (first human in-vivo evidence for the postprandial lactate shuttle): overnight-fasted adults, 75-g OGTT, arterialised sampling, [3-¹³C]lactate and deuterated glucose tracers.
- Name and temporally sequence the two PLS phases: the **enteric** phase (blood lactate rises *before* glucose — intestinal epithelium metabolises glucose to lactate, gut → portal vein → liver/systemic) and the **systemic** phase (a second lactate rise synchronous with glucose — hepatic glucose release → peripheral glycolysis → lactate).
- State the quantitative significance: ≈ 38 % of the 75-g glucose load appears in the circulation as lactate, making lactate a carbon carrier of magnitude comparable to glucose itself; and the conclusions — lactate production is not a hypoxia phenomenon, dietary carbohydrate is routinely distributed via the shuttle, lactate buffers the postprandial glucose surge, and the intestinal epithelium is the first actor in systemic carbohydrate carbon flux.
- Derive the sports-nutrition implications (Brooks 2023): systemic carbohydrate flux follows hexose → lactate → oxidation or liver-glycogen storage, so carbohydrate *form* is secondary (all pass through lactate); lactate salts are GRAS and buffer acidosis; training raises MCT expression and thus carbohydrate tolerance without hyperlactataemia; fructose enters the circulation largely as lactate; ketones offer no advantage when carbohydrate is available.
- Connect the nutrition argument to training: endurance training increases simultaneous production and clearance capacity, so Pro-Tour cyclists sustain power for hours at which untrained individuals would already be hyperlactataemic.
...
**Learning objectives:**
- Define the aerobic threshold (AeS = first lactate rise above baseline ≈ VT1) and the individual anaerobic threshold (IAS ≈ MLSS ≈ VT2/RCP), and state their %VO₂max positions (AeS ≈ 40–65 %, IAS ≈ 60–85 %) (Kindermann 2004).
- Explain why individual thresholds outperform the fixed 4 mmol·L⁻¹ criterion, and why the Stegmann et al. 1981 method — which uses recovery lactate kinetics — is reliable, robust to glycogen depletion, and validated against the MLSS.
- Read a lactate-performance curve: the dual lactate/HR axes, the operational LT and IAS definitions (Wasserman & McIlroy 1964), and the four colour-coded training zones (KB/regenerative, GA1, GA2, EB/threshold).
- Interpret longitudinal curve shifts: a **right shift** (higher wattage at the same lactate) marks improved mitochondrial capacity, MCT1/MCT4 upregulation and clearance; a **left shift** (lactate at lower workloads) marks detraining, overreaching, infection or post-infectious deconditioning.
- Apply the clinical case: in ME/CFS and Long COVID the curve left-shifts even after minimal effort, the IAS can fall below 100 W, and conventional zone-based prescription can be harmful without individualised lactate-guided assessment — and connect this to the post-exertional-malaise physiology of these conditions.
- Integrate curve and shuttle: the exponential rise above IAS is the point where production (fast-glycolytic fibres) outstrips oxidative consumption via the intracellular and cell–cell shuttles, and above IAS lactate acts both as substrate and as lactormone (PGC-1α, VEGF, BDNF, fat-oxidation programs) (Brooks 2018).
- State the key methodological caveats: protocol/modality specificity, resting-lactate variability affecting LT more than IAS, the need for replicated confidence bands, and the HR–lactate bridge to wearable monitoring.
